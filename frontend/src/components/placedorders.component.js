...
export default class PlacedOrders extends Component {
    ...
    onSubmit(e) {
        ...
        axios.post('http://localhost:5000/api/products/' + e.target.id, {
    ...
    componentDidMount(e) {
        // e.preventDefault();
        axios.get('http://localhost:5000/api/products/placed', {
            headers: {'x-auth-token': sessionStorage.getItem('token')}
        })
                            ...
                            this.state.list.map((currentUser, i) => {
                                return (
                                    <tr>
                                        ...
                                        <td><button id = {currentUser._id} type="submit" className = "btn btn-primary" onClick={this.onSubmit}>Dispatch</button></td>
                                    </tr>
                                )
                            })
                        }
